...
class Variant extends ActiveRecord
{
    ...
    {  //str_replace(array('{{','}}','%'),'',$name)
        ...
        return $this->hasMany(Image::class, ['id' => 'image_id'])
            ->viaTable("{{%variant_image}}", [str_replace(array('{{','}}','%'),'',$name) . '_id' => 'id'])
            ->leftJoin("{{%variant_image}}", 'id=image_id')
            ->where(["{{%variant_image}}." . str_replace(array('{{','}}','%'),'',$name) . '_id' => $this->id])
        //    ->andFilterWhere([$name . '_image.enabled' => true])
            ->orderBy(["{{%variant_image}}.position" => SORT_ASC])
            ->indexBy('id');
    }
    ...
    public function getImagesAll()
    {
        ...
        return $this->hasMany(Image::class, ['id' => 'image_id'])
            ->viaTable("{{%variant_image}}", [str_replace(array('{{','}}','%'),'',$name) . '_id' => 'id'])
            ->leftJoin("{{%variant_image}}", 'id=image_id')
            ->where([$name . '_image.' . str_replace(array('{{','}}','%'),'',$name) . '_id' => $this->id])
    ...
    public function getImageEnabled()
    {
        ...
        return $this->_imageEnabled = (new \yii\db\Query())
            ->select(['enabled'])
            ->from("{{%variant_image}}")
            ->where([str_replace(array('{{','}}','%'),'',$name). '_id' => $this->id])
            ->indexBy('image_id')
            ->column();
    }
